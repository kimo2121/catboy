...
const Mint: React.FC = () => {
  ...
  const mintTokens = async () => {
    if (!loginStatus) {
      toast.error("Please connect wallect correctly on Binance Smart Chain");
      return;
    }
    if (count <= 0) {
          ...
          <div className="counter-button-container">
            ...
            <div className="mint-span-details">
              <span style={{ marginRight: "2vw" }}>
                Minted {mintEngineDetail?.totalSupply} /{" "}
                {mintEngineDetail?.maxSupply}
              </span>
              <span> {mintEngineDetail?.mintPrice} BNB per Mint</span>
            </div>
          </div>
        </div>
      </div>
